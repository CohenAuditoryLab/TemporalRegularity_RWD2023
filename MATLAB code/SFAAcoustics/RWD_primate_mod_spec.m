...
corr_spec = 1;                              %Set whether to take mod spec of og spectrogram or quadratically expanded (i.e include correlations)
...
temp_pdf = nan(nvoctypes,length(dwt)); %2022-10-03 trying saving distribution of scaled values (i.e what percentage of the mod spec power is at that temporal modulation when scaled to 1)
...
for itypes=1:nvoctypes
    ...
    temp_pdf(itypes,:) = sum(scaled2plot,1)/sum(scaled2plot,[1,2]);
...
% 
% figure %not worth doing there is no real change in the shape
% for itypes = 1:nvoctypes %Now looking at empirical pdf of modspec
%     
%     subplot(nrow,ncol,itypes)
%     plot(dwt,temp_pdf(itypes,:))
%     
%     title(voctypes_labels{itypes});  
%     
%     if ( fix(itypes/ncol) == (nrow-1))
