...
Same thing as Clutter_runSFA but for when there is clutter in both the training and testing set
...
def run_SFA(cur_pair,vocal_foldername,vocal_pair,clutterfoldername1,clutterfoldername2,  clutterfiletemplate,run_nulls):
    ...
    mode = 'ICA'
    SFA_save_score = np.zeros((5,snr_values.size,snr_values.size)) 
    Baseline_save_score = np.zeros((5,snr_values.size,snr_values.size)) 
    ...
    for a_round in range(0,5): #rounds are for averaging over for each set of snr (i.e. each snr is done 10 times)
        ...
        for iteration in range(0,snr_values.size): #iterations for doing each snr
             ...
             #Note 2022-08-26 both clutter files will have the same name they will just be in different folders
             ...
             noise_file_train = clutterfoldername1 / clutter_file
             noise_file_test = clutterfoldername2 / clutter_file
             #Same as above now switch to windows path
             noise_file_train = PureWindowsPath(noise_file_train)
             noise_file_test = PureWindowsPath(noise_file_test)
            ...
            ## Parameters for vocalization and noise pre processing
             ...
             gfb = g.GammatoneFilterbank(samplerate= 50000, order=4, density = 1.0, startband = -21, endband = 21, normfreq = 2200) #sets up parameters for our gammatone filter model of the cochlea.
            ...
            #new number of presentations, just 1 for one-shot learning
             ...
             apply_noise = True #toggle for applying noise to training set
                ## Parameters for testing data
                
                
             test_noise = True #Toggle for adding noise to testing set
             ...
             baseline_features = 'all' #how many features the baseline by itself gets to use
            ...
            ##Load in and adjust noise power accordingly to sigal to noise ratio
             ...
             if(load_noise):
                noise1, _ = sf.read(noise_file_train)
                noise2, _ = sf.read(noise_file_test)
            
             print('Noises loaded...')
             print('Ready for preprocessing.')
            
             if noise is not None:
                noise1 = scale_noise(vocalizations,noise1,signal_to_noise_ratio) #scales based on average power
                noise1 = noise1[:noiselen]
             print('Noise Scaled...')
            ...
            ## Apply Gammatone Transform to signal and noise
             ...
             if noise is not None:
                noise_transformed = gamma_transform(noise1, gfb)
...
#%% NEXT LOOP GOES HERE WHERE WE LOOP THROUGH THE DIFFERENT SNRS FOR TESTING   
            #For the moment have the 2nd SNR match the 1st rather than looping through all of the possible 2nd SNRs for each first SNR.
             for iter2 in range(0,snr_values.size):
                 print(iter2)
#%%         Test Data
                 ...
                 if(test_noise):
                    #signal_to_noise_ratio = snr_values[iter2] #Again temp removal of this line so they just match.
                    noise2 = scale_noise(vocalizations,noise2,snr_values[iter2]) #scales based on average power
                    noise2 = noise2[:noiselen]
                    print('Noise Scaled...')
                    print('Ready For Gammatone Transform')
                    noise_transformed = gamma_transform(noise2, gfb)
                    print('Noise Transformed...')
    ...
    #%% Apply SFA to Test Data, also toggles for using second layer
                ...
                #2022-09-05    #Updated to see if having a larger training set will help prevent failure for convergence that happens sometimes for quad method.
                 ...
                 SFA_save_score[a_round,iteration,iter2] = np.mean(cv_sfa['test_score']) #take average of all CV folds as the score for that round for that SNR
                 ...
                 Baseline_save_score[a_round,iteration,iter2] = np.mean(cv_baseline['test_score']) #same thing for the baseline classifier, this should have consistent performance, but can use the variance present in this as a check on the classifier
